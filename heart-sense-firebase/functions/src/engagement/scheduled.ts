...
/**
 * Scheduled: Test notification at 2:30 PM Pacific (for testing).
 * Sends one engagement alert to the user ID in config/test_notification (set via scheduleTestNotificationAt230).
 * Removes the config doc after sending so it does not repeat every day.
 */
export const testNotificationAt230 = onSchedule(
  {
    schedule: "30 14 * * *", // Every day at 2:30 PM
    timeZone: "America/Los_Angeles",
  },
  async () => {
    logger.info("Running test notification at 2:30 PM");

    try {
      const configRef = db.collection("config").doc("test_notification");
      const configDoc = await configRef.get();

      if (!configDoc.exists) {
        logger.info("No test_notification config found, skipping.");
        return;
      }

      const {userId} = configDoc.data() as {userId: string};
      if (!userId) {
      ...
      await createEngagementAlert(
        userId,
        "inactivity_warning",
        "Test notification (2:30 PM)",
        "This is a test notification sent at 2:30 PM. If you see this, the notification flow works!",
        "low",
        {type: "test_230"},
        24
      );
      ...
      logger.error(`Error in test notification at 2:30: ${error}`);
    }
  }
);
